...
const Renderer = {
  ...
  slider_v(b,em){
    const p=b.props||{};
    const s=`width:${p.width||'100%'};height:${p.height||'400px'};overflow-y:auto;display:flex;flex-direction:column;gap:${p.gap||12}px;${p.bg_color?'background:'+p.bg_color+';':''}padding:${p.padding||0}px;box-sizing:border-box;scrollbar-width:thin`;
    const ch=this.render(b.children||[],em);
